...
<div id="content">
	<div id="wrapper">
     ...
     <span class="Validform_checktip">标题名称在3~50位字符,且不为空</span>
							...
							<div class="form">
								<link rel="stylesheet"
									href="plug-in/uploadify/css/uploadify.css" type="text/css"></link>
						...
						$('#file_upload').uploadify({
							...
							onUploadSuccess : function(file, data, response) {
								...
								if(d.msg=="文件添加成功"){
									...
									win.tip('上传完成');frameElement.api.close();	
								}else{
									var win = frameElement.api.opener;
									win.tip('请勿重复上传');
								}
					...
					function cancel() {
						$('#file_upload').uploadify('cancel', '*');
								...
								<span id="file_uploadspan"><input type="file"
									name="fiels" id="file_upload" /></span>
							</div>
		...
		$("#formobj").Validform({
			...
			callback : function(data) {
				...
				if (data.success == true) {
					...
					if (data.responseText == '' || data.responseText == undefined) $("#formobj").html(data.msg);else $("#formobj").html(data.responseText);
					return false;
				}
