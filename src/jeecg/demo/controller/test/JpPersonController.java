...
public class JpPersonController extends BaseController {
	...
	public void exportXls(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// ---------begin
			// --------Author:邢双阳---date：2013-5-14---for：firefox浏览器下载时文件标题乱码
			codedFileName = "用户信息";
			// 根据浏览器进行转码，使其支持中文文件名
	...
	public AjaxJson implXls(HttpServletRequest request, HttpServletResponse response) {
		...
		// -- update-begin--Author:lihuan Date:20130509
		// for：增加判断格式不正确的情况，防止格式不正确时抛出异常到页面
		try {
	...
	// 统一的Excel上传导入方式
	@RequestMapping(params = "importExce", method = RequestMethod.POST)
	@ResponseBody
	public AjaxJson importExce(HttpServletRequest request, HttpServletResponse response) {
		AjaxJson j = new AjaxJson();
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			List<JpPersonEntity> listPersons;
			try {
				listPersons = (List<JpPersonEntity>) ExcelUtil.importExcelByIs(file.getInputStream(),
						JpPersonEntity.class);
				for (JpPersonEntity person : listPersons) {
					person.setId(UUID.randomUUID().toString());
					jpPersonService.save(person);
				}
	...
	@RequestMapping(params = "importExcel1", method = RequestMethod.POST)
	@ResponseBody
	public AjaxJson importExcel1(HttpServletRequest request, HttpServletResponse response) {
		AjaxJson j = new AjaxJson();
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			// List<JpPersonEntity> listPersons;
			try {
				List<ZSZzc> listZzcs = (List<ZSZzc>) ExcelUtil.importExcelByIs(file.getInputStream(), ZSZzc.class);
				for (ZSZzc zzc : listZzcs) {
					zzc.setId(UUID.randomUUID().toString());
					systemService.save(zzc);
					// jpPersonService.save(person);
				}
				j.setMsg("文件导入成功！");
			} catch (IOException e) {
